...
import { getCitations, deleteCitationById } from 'apiSdk/citations';
import { CitationInterface } from 'interfaces/citation';

type ColumnType = ColumnDef<CitationInterface, unknown>;

interface CitationListPageProps {
...
export function CitationListPage(props: CitationListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CitationInterface>>(
    () => `/citations?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CitationInterface) => {
    if (hasAccess('citation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/citations/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'text_content', header: 'Text Content', accessorKey: 'text_content' },
    { id: 'image_content', header: 'Image Content', accessorKey: 'image_content' },
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/citations/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
          ...
          {hasAccess('citation', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/citations/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('citation', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="citation" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Citations
        </Text>
        {hasAccess('citation', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/citations/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
