...
import { getCitationById, updateCitationById } from 'apiSdk/citations';
...
function CitationEditPage() {
  ...
  const handleSubmit = async (values: CitationInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateCitationById(id, values);
      mutate(updated);
      resetForm();
