...
import { getCitationById } from 'apiSdk/citations';
import { CitationInterface } from 'interfaces/citation';

function CitationViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<CitationInterface>(
    () => (id ? `/citations/${id}` : null),
    () =>
      getCitationById(id, {
        ...
        {isLoading ? (
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                <FormListItem label="Text Content" text={data?.text_content} />

                <FormListItem label="Image Content" text={data?.image_content} />
